...
from clockify_mcp.telemetry import (
    DETAIL_METADATA,
    InstrumentedFastMCP,
    Telemetry,
    build_telemetry,
...
def test_noop_tool_span_is_a_passthrough_context_manager():
    telemetry = build_telemetry(_config())
    with telemetry.tool_span("get_workspace", {"id": 1}):
        pass  # must not raise


def test_noop_client_span_yields_a_recorder():
    telemetry = build_telemetry(_config())
    with telemetry.client_span("GET", "workspaces", {"id": 1}) as span:
...
def test_tool_span_has_genai_attributes(otel):
    ...
    with telemetry.tool_span("get_workspace", {"id": 21574}):
        pass
    spans = exporter.get_finished_spans()
    assert len(spans) == 1
    span = spans[0]
    assert span.name == "execute_tool get_workspace"
    assert span.attributes["gen_ai.operation.name"] == "execute_tool"
    assert span.attributes["gen_ai.tool.name"] == "get_workspace"
    assert span.attributes["mcp.tool.name"] == "get_workspace"
...
def test_client_span_has_http_attributes(otel):
    telemetry, exporter, _ = otel
    with telemetry.client_span("GET", "workspaces.by.user", {"id": 18}) as span:
        span.set_status_code(200)
    span = exporter.get_finished_spans()[0]
    assert span.attributes["http.request.method"] == "GET"
    assert span.attributes["clockify.endpoint"] == "workspaces.by.user"
...
def test_ids_tier_captures_argument_ids():
    ...
    with telemetry.tool_span("get_workspace", {"id": 21574, "details": True}):
        pass
    attrs = exporter.get_finished_spans()[0].attributes
    assert attrs["mcp.tool.argument.id"] == 21574
    assert "mcp.tool.argument.details" not in attrs


def test_full_tier_captures_capped_payload():
    telemetry, exporter = _otel_with_detail("full")
    with telemetry.tool_span("list_workspaces", {"name": "x" * 5000}):
        pass
    attrs = exporter.get_finished_spans()[0].attributes
...
def test_token_is_redacted_at_full_tier():
    ...
    with telemetry.tool_span("list_workspaces", {"note": "uses SECRET-TOKEN inside"}):
        pass
    attrs = exporter.get_finished_spans()[0].attributes
...
def test_token_is_redacted_at_ids_tier():
    ...
    with telemetry.tool_span("get_workspace", {"id": "SECRET-TOKEN"}):
        pass
    attrs = exporter.get_finished_spans()[0].attributes
...
def test_resource_has_default_service_name(monkeypatch):
    ...
    assert telemetry.resource_service_name() == "clockify-mcp"
...
def test_client_span_has_server_address(otel):
    ...
    with telemetry.client_span("GET", "workspaces", {"id": 1}) as span:
        span.set_status_code(200)
    attrs = exporter.get_finished_spans()[0].attributes
    assert attrs["server.address"] == "api.clockify.me"
...
def test_client_span_records_size_and_item_count(otel):
    ...
    attrs = exporter.get_finished_spans()[0].attributes
    assert attrs["clockify.response.item_count"] == 3
    assert attrs["clockify.response.size"] > 0


def test_client_span_size_redacts_token():
    telemetry, exporter = _otel_with_detail("metadata", token="SECRET-TOKEN")
    with telemetry.client_span("GET", "workspaces", None) as span:
        span.set_status_code(200)
        span.set_result({"note": "SECRET-TOKEN here"})
    attrs = exporter.get_finished_spans()[0].attributes
    assert "clockify.response.item_count" not in attrs  # dict, not a list
    assert attrs["clockify.response.size"] > 0
...
def test_logger_provider_configured_with_non_propagating_handler():
    ...
    pkg_logger = logging.getLogger("clockify_mcp")
